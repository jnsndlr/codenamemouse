...
## One thing at a time. A mouse holding a wedge does not pick up another, and a carrier does
## not pick up cheese -- the two errands are meant to compete for the same mouse.
func _check_paws_are_full() -> void:
	print("\n-- one thing in the paws at a time")
	var cache := _fresh_cache(_stores_of(Team.BLUE) + Vector3(6.0, 0.0, 0.0), 4)
	await _park(_blue, cache.global_position)
	var left: int = cache.wedges
	await _park(_blue, cache.global_position, 10)
	_ok("a full-pawed mouse takes no second wedge", cache.wedges == left,
		"cache went %d -> %d while the mouse was already loaded" % [left, cache.wedges])
	...
	await _park(_blue, cache.global_position, 10)
	_ok("a banner carrier takes no cheese", _blue.get_carried_cheese() == 0)
	_blue.release_carry()
